...
1. $\forall n\in\mathbb{Z},\ n\text{ odd}\Rightarrow n^2\text{ odd}$.
2. If $n=2k+1$, then
...
1. For any `x`, $x\notin A\cap B$ exactly when $x\notin A$ or $x\notin B$; therefore
$(A\cap B)^c=A^c\cup B^c$.
2. $x\in A\setminus(B\cup C)$ means $x\in A$, $x\notin B$, and $x\notin C$,
which is exactly $x\in(A\setminus B)\cap(A\setminus C)$.
...
1. The degree sum is $8\cdot3+4\cdot2=32$, so the graph has
   $32/2=16$ edges.
