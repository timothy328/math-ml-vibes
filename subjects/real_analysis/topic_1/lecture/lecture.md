...
converges to `L` if for every `ε>0` there exists `N` such that `n≥N` implies
`|a_n-L|<ε`. The definition says that all sufficiently late terms lie in
every prescribed neighborhood of `L`.
...
The limit `lim_{x→a} f(x)=L` means: for every `ε>0`, there exists `δ>0` such
that `0<|x-a|<δ` implies `|f(x)-L|<ε`.

Proof workflow:

1. Start with the desired bound `|f(x)-L|<ε`.
2. Bound factors involving `x`.
3. Choose a neighborhood restriction and then choose `δ`.
...
`f` is continuous at `a` when `lim_{x→a}f(x)=f(a)`. Continuity preserves
limits under composition and ensures that a continuous function on a compact
interval attains its maximum and minimum.

Uniform continuity uses one `δ` for all points in a domain:
...
Lipschitz continuity, `|f(x)-f(y)|≤K|x-y|`, gives a quantitative stability
